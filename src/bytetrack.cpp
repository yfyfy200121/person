...
cv::Mat letterbox(const cv::Mat& src, int target_width, int target_height, float& scale, int& pad_w, int& pad_h) {
    ...
    scale = std::min(target_width / (src_w * 1.0f), target_height / (src_h * 1.0f));
    ...
    cv::Mat output(target_height, target_width, src.type(), cv::Scalar(114, 114, 114));  // 灰色背景
...
enum RunMode { DETECTING, TRACKING };
...
int main(int argc, char** argv) 
{
    ...
    */
    ...
    RunMode mode     = DETECTING;
    int     lost_cnt = 0;
    const int max_lost = 30; 
    int num_frames = 0;
    int total_ms = 0;
    float out_scales[3] = {0};
    int32_t out_zps[3] = {0};
   
int detect_frame_count = 0;
const int DETECT_FRAME_THRESHOLD = 90;
std::vector<Object> candidate_persons;

while (true) {
    // 1. 读取帧
    cv::Mat raw_img;
    if (!cap.read(raw_img)) break;
    ...
    int pad_w = 0, pad_h = 0;
    cv::Mat pr_img = letterbox(raw_img, INPUT_W, INPUT_H, resize_scale, pad_w, pad_h);
    ...
    for (auto& obj : objects) {
        obj.rect.x = int((obj.rect.x - pad_w) / resize_scale);
        obj.rect.y = int((obj.rect.y - pad_h) / resize_scale);
        obj.rect.width = int(obj.rect.width / resize_scale);
        obj.rect.height = int(obj.rect.height / resize_scale);
    }

    // 7. 提取 person
    std::vector<Object> persons;
    for (auto& obj : objects) {
        if (obj.label == 0 && obj.prob > 0.3f) {
    ...
    if (mode == DETECTING) {
        ...
        if (!persons.empty()) {
            auto best = *std::max_element(persons.begin(), persons.end(),
                [](const Object& a, const Object& b) {
                    return a.prob < b.prob;
                });
            candidate_persons.push_back(best);
        }

        if (detect_frame_count >= DETECT_FRAME_THRESHOLD && !candidate_persons.empty()) {
            Object target = candidate_persons.back();
            std::vector<Object> init_target = { target };
            output_stracks = tracker.update(init_target);

            mode = TRACKING;
            lost_cnt = 0;
            detect_frame_count = 0;
            candidate_persons.clear();
        }

    } else {
        
        output_stracks = tracker.update({persons});
        if (output_stracks.empty() && ++lost_cnt > max_lost) {
            mode = DETECTING;
            detect_frame_count = 0;
            candidate_persons.clear();
        } else if (!output_stracks.empty()) {
            lost_cnt = 0;
        }
    }
    ...
    if (mode == DETECTING) {
        if (!persons.empty()) {
            ...
            cv::rectangle(raw_img, best.rect, cv::Scalar(0, 255, 0), 2);  // 绿色框
            cv::putText(raw_img,
                        cv::format("P:%.2f", best.prob),
                        cv::Point(best.rect.x, best.rect.y - 5),
        ...
        if (!output_stracks.empty()) {
            const auto& tr = output_stracks[0];
            int x = int((tr.tlwh[0] - pad_w) / resize_scale);
            int y = int((tr.tlwh[1] - pad_h) / resize_scale);
            int w = int(tr.tlwh[2] / resize_scale);
            int h = int(tr.tlwh[3] / resize_scale);
            cv::Rect bbox(x, y, w, h);
            cv::rectangle(raw_img, bbox, cv::Scalar(0, 0, 255), 2);  // 红色框
            cv::putText(raw_img,
                        cv::format("ID:%d", tr.track_id),
                        cv::Point(x, y - 5),
                        cv::FONT_HERSHEY_SIMPLEX,
                        0.6,
                        cv::Scalar(0, 0, 255),
    ...
    // 10. 显示信息
    cv::putText(raw_img,
                cv::format("frame:%d mode:%s",
                           num_frames,
                           mode == DETECTING ? "DETECT" : "TRACK"),
                cv::Point(10, 30),
                cv::FONT_HERSHEY_SIMPLEX,
                0.6,
                cv::Scalar(0, 0, 255),
